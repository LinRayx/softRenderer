...
//    Util::printVec4(view * vec4(world_pos, 1));
    ...
    Util::printVec4(lightSpacePos);
    fragmentData.lightSpacePos[0] = (lightSpacePos[0]/lightSpacePos[3] + 1.f)/2;
    fragmentData.lightSpacePos[1] = (lightSpacePos[1]/lightSpacePos[3] + 1.f)/2;
    fragmentData.lightSpacePos[2] = (lightSpacePos[2]/lightSpacePos[3] + 1.f)/2;
...
float PhoneShader::ShadowCalculation(IN FragmentData &fragmentData, float bias) {
    ...
    if (shadowMap.size() > 0) {
        vec3 clostDepth = textureShadow(vec2(x, y), shadowMap, SCREEN_WIDTH, SCREEN_HEIGHT);
//        std::cout << x << " " <<y << " " << fragmentData.lightSpacePos[2] <<" " << clostDepth[0] <<  std::endl;
        if (fragmentData.lightSpacePos[2]-bias > clostDepth[0]) {
            shadow = 1;
        }
    }
    return shadow;
